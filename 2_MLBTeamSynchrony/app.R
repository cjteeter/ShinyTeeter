        ...
        sidebarLayout(
                sidebarPanel(
                        selectizeInput(inputId = "season_choice", label = tags$h4("Select Season:"),
                                    ...
                                    selected = NULL,
                                    multiple = T,
                                    options = list(placeholder = 'Select a season', maxItems = 1)),#onInitialize = I('function() { this.setValue(""); }'))),
                        hr(),
                        selectizeInput(inputId = "team_choice", label = tags$h4("Select Team:"),
                                    choices = teams_df$Full.Name,
                                    selected = NULL,
                                    multiple = T,
                                    options = list(placeholder = 'Select a team', maxItems = 1)), #onInitialize = I('function() { this.setValue(""); }'))),
                        hr(),
...
server <- function(input, output, session) {
        ...
        observeEvent(input$season_choice, { updateSelectizeInput(session, inputId = "team_choice", choices = update_teams(teams_df, input$season_choice)) }, ignoreInit = T)
        
        # First Tab - RS_RA Figure -----------------------------------
        eventPlot_rsra <- eventReactive(input$generate, {
                                        rs_ra_roll_plot(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice)}, input$team_choice)
        })
        ...
        link1 <- eventReactive(input$generate, as.character({ paste('https://www.baseball-reference.com/teams/',
                                                                    { if (input$team_choice %in% c("Anaheim Angels", "Florida Marlins", "Montreal Expos", "Tampa Bay Devil Rays")) {
                                                                            teamcode_for_link(input$team_choice) } else { teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)] }},
                                                                    '/', input$season_choice, '-schedule-scores.shtml', sep = "") }))
        eventLink1 <- eventReactive(input$generate, 
                                    as.character({ paste(input$season_choice, input$team_choice, 'schedule and results on Baseball-Reference.com') }))
        
        output$fig_link_rsra <- renderUI({ tags$a(href = link1(), eventLink1(), target = '_blank') })
        
        eventPlot_rsra_dl <- function(){ rs_ra_roll_plot(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice)}, input$team_choice) }
        
        output$dl_fig_rsra <- downloadHandler(filename = function() { paste('MLBsyncrony_', input$season_choice, '_', teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], '_RS-RA_plot_dl-', Sys.Date(), '.png', sep="") },
                                         content = function(file) {
                                                 ggsave(file, width = 9, height = 6, plot = eventPlot_rsra_dl(), device = "png")
                                         })
        
        # Second Tab - RDiff Figure -----------------------------------
        eventPlot_rdiff <- eventReactive(input$generate, {
                                        rdiff_roll_plot(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice)}, input$team_choice)
        })
        ...
        eventPlot_rdiff_dl <- function(){ rdiff_roll_plot(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice)}, input$team_choice) }
        
        output$dl_fig_rdiff <- downloadHandler(filename = function() { paste('MLBsyncrony_', input$season_choice, '_', teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], '_RDiff_plot_dl-', Sys.Date(), '.png', sep="") },
                                         content = function(file) {
                                                 ggsave(file, width = 9, height = 6, plot = eventPlot_rdiff_dl(), device = "png")
                                         })
        
        # Third Tab - Data Table for Figure -----------------------------------
        eventTable1 <- eventReactive(input$generate, {
                figdata_tbl(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice)})
        })
        
        output$figure_table <- renderDT({if (input$table_widelong == 'wide') { eventTable1() }
        ...
        eventTable1_dl <- function(){ figdata_tbl(master_data, input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], {ifelse(input$season_choice == curSeason, input$rolling_choice_curS, input$rolling_choice) })
        }
        
        output$dl_tbl1 <- downloadHandler(filename = function() { paste('MLBsyncrony_', input$season_choice, '_', teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], '_DataTable-', {if (input$table_widelong == 'wide') { 'wide' } else { 'long' }}, '_dl-', Sys.Date(), '.csv', sep="") },
                                                content = function(file) {
        ...
        eventTable2 <- eventReactive(input$generate, { standings_tbl(master_data, teams_df, input$season_choice)
        })
        
        output$standings_table <- renderDT({ eventTable2 <- eventTable2()
        ...
        eventTable2_dl <- function(){ standings_tbl(master_data, teams_df, input$season_choice) }
        
        link2 <- eventReactive(input$generate, as.character({ paste('https://www.baseball-reference.com/leagues/MLB/', input$season_choice, '-standings.shtml', sep = "") }))
        eventLink2 <- eventReactive(input$generate, 
                            as.character({ paste(input$season_choice, 'Standings on Baseball-Reference.com') }))
        output$stnd_link <- renderUI({ tags$a(href = link2(), eventLink2(), target = '_blank') })

        output$dl_tbl2 <- downloadHandler(filename = function() { paste('MLBsyncrony_', input$season_choice, '_Standings-plus_dl-', Sys.Date(), '.csv', sep="") },
                                  content = function(file) {
