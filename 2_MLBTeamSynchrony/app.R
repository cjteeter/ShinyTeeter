...
ui <- fluidPage(theme = shinytheme('spacelab'),
        ...
        p("App created by ", tags$a(href = "http://www.cteeter.ca", 'Chris Teeter', target = '_blank'), " in November 2017.", HTML("&bull;"), "Follow Chris on Twitter:", tags$a(href = "https://twitter.com/c_mcgeets", tags$i(class = 'fa fa-twitter'), target = '_blank'),
          HTML("&bull;"), "Find the code on Github:", tags$a(href = "https://github.com/cjteeter", tags$i(class = 'fa fa-github', style = 'color:#5000a5'), target = '_blank'), style = "font-size: 85%"),
...
server <- function(input, output, session) {
        ...
        observeEvent(input$season_choice, { updateSelectizeInput(session, inputId = "team_choice", choices = update_teams(isolate(input$season_choice), teams_df)) })
        
        # First Tab - RS_RA Figure -----------------------------------
        eventPlot_rsra <- eventReactive(input$generate, isolate({
                                        rs_ra_roll_plot(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice, input$team_choice)
        }))
        ...
        eventPlot_rsra_dl <- function(){ isolate(rs_ra_roll_plot(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice, input$team_choice))}
        
        output$dl_fig_rsra <- downloadHandler(filename = function() { paste('MLBsyncrony_', isolate(input$season_choice), '_', teams_df$Team.Code[teams_df$Full.Name == team_flip(isolate(input$team_choice))], 'figure_dl-', Sys.Date(), '.png', sep="") },
        ...
        eventPlot_rdiff <- eventReactive(input$generate, isolate({
                                        rdiff_roll_plot(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice, input$team_choice)
        }))
        ...
        eventPlot_rdiff_dl <- function(){ isolate(rdiff_roll_plot(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice, input$team_choice))}
        
        output$dl_fig_rdiff <- downloadHandler(filename = function() { paste('MLBsyncrony_', isolate(input$season_choice), '_', teams_df$Team.Code[teams_df$Full.Name == team_flip(isolate(input$team_choice))], 'figure_dl-', Sys.Date(), '.png', sep="") },
                                         content = function(file) {
        ...
        eventTable1 <- eventReactive(input$generate, isolate({
                figdata_tbl(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice)
        }))
        ...
        eventTable1_dl <- function(){ isolate(figdata_tbl(input$season_choice, teams_df$Team.Code[teams_df$Full.Name == team_flip(input$team_choice)], input$rolling_choice))
        }
        ...
        eventTable2 <- eventReactive(input$generate, isolate({ standings_tbl(input$season_choice)
        }))
        ...
        eventTable2_dl <- function(){ isolate(standings_tbl(input$season_choice)) }
