...
figdata_tbl <- function(year, team_code, roll_num) {
        team_data <- master_data %>% filter(Year == year, Tm_Current == team_code)
...
standings_tbl <- function(year) {
        ...
        for (t in 1:length(teams_df$Team.Code)) {
                team_data <- master_data %>%
                                filter(Year == year, Tm_Current == teams_df$Team.Code[t])
                
                next_row <- data.frame("Year" = year,
                              "Team" = team_data$Tm[1],
                              "League" = ifelse(year > 2012, teams_df$League_Current[teams_df$Team.Code == teams_df$Team.Code[t]], teams_df$League_pre2013[teams_df$Team.Code == teams_df$Team.Code[t]]),
                              "Division" = ifelse(year > 2012, teams_df$Division_Current[teams_df$Team.Code == teams_df$Team.Code[t]], teams_df$Division_pre2013[teams_df$Team.Code == teams_df$Team.Code[t]]),
                              "G" = nrow(team_data), 
